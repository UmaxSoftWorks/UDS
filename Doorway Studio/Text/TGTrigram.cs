    ...
    class TGTrigram : TextGenerator
    {
        ...
        public override void Load(string Input)
        {
            ...
            for (int i = 0; i < Words.Length; i++)
            {
                ...
                {
                    //All other words
                    WordsProbability[i] = GetWordsCount(i-2, i - 1, i) / ((double)GetWordsCount(i - 1, i) * (double)Words.Length);
                }
            }
        }
        ...
        private double GetWordsCount(int PreviousPreviousWordIndex, int PreviousWordIndex, int WordIndex)
        {
            double count = 0;

            for (int i = 2; i < Words.Length; i++)
            {
                if (WordsHash[PreviousPreviousWordIndex] == WordsHash[i - 2] && WordsHash[PreviousWordIndex] == WordsHash[i - 1] && WordsHash[WordIndex] == WordsHash[i])
                {
                    count++;
                }
            }
